//  Recommended first step - search and replace "mcp23017_" with your own plugin name
...
*/
function mcp23017_update_onload() {
...
jQuery(document).ready(mcp23017_update_onload);
...
/*
    ...
    Rename this function and references to it so it doesn't conflict with functions defined by any other plugins
*/
function mcp23017_update_periodically() {
    /* your custom UI change code here */
    // You can scope these changes to a specific page, look up in urls.py (e.g. home=="/", programs="/vp", runonce ="/vr" etc.)
    if (window.location.pathname == "/") {
...
jQuery(document).ready(function(){
    ...
    mcp23017_update_periodically();  // Call once now
    // Update with your specific function name and the requested update interval
    setInterval(mcp23017_update_periodically, 1000*60);  // Can call again every <n> milliseconds
});
...
*/
function mcp23017_update_schedule() {
    /* your custom UI change code here */
    ...
    $.get( "/mcp23017-data", {"date" : toXSDate(displayScheduleDate)}, function( data ) {
...
jQuery(document).ready(function(){
    ...
    if (window.location.pathname == "/") {
        ...
        if ($('#displayScheduleDate').length > 0) {
            ...
            observer = new MutationObserver(mcp23017_update_schedule);
